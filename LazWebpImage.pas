...
uses LclIntF, LMessages, LclType, SysUtils, lazlibwebp124, {ExtendedImageUtils,} IntfGraphics,
  Graphics, Classes, Math, ExtCtrls, Dialogs, Forms, GraphType, Types;
...
//function GetBrushOrgEx(dc: HDC; var point: TPoint): LONGBOOL; e
...
begin
  ...
  begin
    ...
    try
      FillChar(bmih, sizeof(BITMAPINFOHEADER), 0);
      bmih.bmiHeader.biSize := sizeof(BITMAPINFOHEADER);
  	  bmih.bmiHeader.biWidth := FWidth;
  	  bmih.bmiHeader.biHeight := -FHeight;
  	  bmih.bmiHeader.biPlanes := 1;
  	  bmih.bmiHeader.biBitCount := 32;
  	  bmih.bmiHeader.biCompression := BI_RGB;
  	  bmih.bmiHeader.biSizeImage := FWidth * FHeight * sizeof(RGBQUAD);
  		bmfh.bfType := $4D42;
  		bmfh.bfSize := sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) +
        (FWidth * FHeight * sizeof(TRGBquad));
  		bmfh.bfOffBits := sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER);
  		bmfh.bfReserved1 := 0;
  		bmfh.bfReserved2 := 0;
      mb.Write(bmfh, sizeof(BITMAPFILEHEADER));
      mb.Write(bmih, sizeof(BITMAPINFOHEADER));
      i:= Length(BlendedBits);
      mb.Write((@BlendedBits[0])^, i);
      mb.Position:= 0;
      b.LoadFromStream(mb);
